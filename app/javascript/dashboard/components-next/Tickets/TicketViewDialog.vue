...
const title = computed(() =>
  isRename.value
    ? t('TICKETS.VIEWS.DIALOG.RENAME_TITLE')
    : t('TICKETS.VIEWS.DIALOG.CREATE_TITLE')
);
...
<template>
  <Dialog
    ref="dialogRef"
    :title="title"
    :confirm-button-label="t('TICKETS.VIEWS.DIALOG.SAVE')"
  ...
  >
    <div class="flex flex-col gap-4">
      <Input
        v-model="name"
        :placeholder="t('TICKETS.VIEWS.DIALOG.NAME_PLACEHOLDER')"
        autofocus
      />
      <div v-if="!isRename" class="flex flex-col gap-2">
        <span class="text-sm text-n-slate-11">
          {{ t('TICKETS.VIEWS.DIALOG.TYPE_LABEL') }}
        </span>
        <div class="grid grid-cols-2 gap-2">
          ...
          >
            ...
            {{ t(`TICKETS.VIEW_TYPES.${type.toUpperCase()}`) }}
          </button>
        </div>
      </div>
    </div>
