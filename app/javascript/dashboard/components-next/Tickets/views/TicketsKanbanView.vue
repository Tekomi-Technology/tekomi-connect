...
import { positionBetween } from '../constants';
...
const ticketsInStage = stageId => {
  const stageTickets = props.tickets.filter(
    ticket => ticket.stageId === stageId
  );
  if (!props.isManualOrder) return stageTickets;
  return [...stageTickets].sort((a, b) => a.position - b.position);
};
    ...
    <template v-else>
      ...
      >
        <header class="flex items-center gap-2 px-1 pb-2 text-sm">
          ...
          <span
            v-if="column.stage.sla"
            v-tooltip.top="$t('TICKETS.SLA.STAGE_TARGET')"
            class="ltr:ml-auto rtl:mr-auto text-n-slate-11 truncate"
          >
            {{ slaLabel(column.stage) }}
          </span>
        </header>
        <div
