...
import TicketViewsAPI from 'dashboard/api/ticketViews';
...
export const useTicketViewsStore = defineStore('ticketViews', {
  state: () => ({
    records: [],
  ...
  actions: {
    async fetch(pipelineId) {
      ...
      try {
        const { data } = await TicketViewsAPI.get(pipelineId);
    ...
    async create(view) {
      ...
      try {
        const { data } = await TicketViewsAPI.create(toRequestPayload(view));
    ...
    async update(id, attributes) {
      this.uiFlags.isSaving = true;
      try {
        const { data } = await TicketViewsAPI.update(
          id,
    ...
    async delete(id) {
      try {
        await TicketViewsAPI.delete(id);
