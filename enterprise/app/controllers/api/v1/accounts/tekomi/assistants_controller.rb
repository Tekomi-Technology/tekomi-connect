class Api::V1::Accounts::Tekomi::AssistantsController < Api::V1::Accounts::BaseController
  ...
  def playground
    response = if tekomi_v2_enabled?
                 Tekomi::Assistant::AgentRunnerService.new(assistant: @assistant, source: 'playground').generate_response(
                   message_history: playground_message_history
                 )
               else
                 Tekomi::Llm::AssistantChatService.new(assistant: @assistant, source: 'playground').generate_response(
  ...
  def tools
    assistant = Tekomi::Assistant.new(account: Current.account)
    @tools = assistant.available_agent_tools
  end

  def metrics
    render json: Tekomi::AssistantStatsBuilder.new(@assistant, params[:range], params[:timezone_offset]).metrics
  end

  def faq_stats
    builder = Tekomi::AssistantStatsBuilder.new(
      @assistant,
      suggestions_scope: Tekomi::FaqSuggestionFinder.new(Current.user, Current.account).perform
    )
  ...
  def summary
    window = Tekomi::AssistantStatsWindow.new(params[:range], params[:timezone_offset])
  ...
  def drilldown
    return head :unprocessable_entity unless Tekomi::AssistantDrilldownBuilder.supported_metric?(params[:metric])

    render json: Tekomi::AssistantDrilldownBuilder.new(@assistant, drilldown_params).build
  end
  ...
  def cached_or_generated_summary(window, stats)
    ...
    result = Tekomi::OverviewSummaryService.new(
  ...
  def summary_cache_key(range)
    "tekomi_overview_summary/#{@assistant.id}/#{Current.user.id}/#{range}/#{Date.current}"
  end
  ...
  def account_assistants
    @account_assistants ||= Tekomi::Assistant.for_account(Current.account.id)
  end

  def assistant_params
    assistant_config_attributes = [
      :product_name, :feature_faq, :feature_memory, :feature_citation,
      :feature_contact_attributes, :feature_general_knowledge, :welcome_message, :handoff_message,
      :resolution_message, :instructions, :temperature, :auto_resolve_mode,
      :response_window
    ]
    if Current.account.feature_enabled?('tekomi_integration_v2')
  ...
  # pass it through raw and let Tekomi::AudienceValidator enforce validity.
  ...
  def tekomi_v2_enabled?
    @assistant.account.feature_enabled?('tekomi_integration_v2')
  end
end
