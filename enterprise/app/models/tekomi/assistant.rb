...
# Indexes
#
#  index_tekomi_assistants_on_account_id  (account_id)
#
class Tekomi::Assistant < ApplicationRecord
  DESCRIPTION_LENGTH_LIMIT = 500
  CITATION_SOURCES_STATE_KEY = :tekomi_v2_citation_sources
  ...
  include Avatarable
  include Concerns::TekomiToolsHelpers
  include Concerns::Agentable

  self.table_name = 'tekomi_assistants'

  belongs_to :account
  has_many :documents, class_name: 'Tekomi::Document', dependent: :destroy_async
  has_many :responses, class_name: 'Tekomi::AssistantResponse', dependent: :destroy_async
  has_many :faq_suggestions, class_name: 'Tekomi::FaqSuggestion', dependent: :destroy_async
  has_many :tekomi_inboxes,
           class_name: 'TekomiInbox',
           foreign_key: :tekomi_assistant_id,
           dependent: :destroy_async
  has_many :inboxes,
           through: :tekomi_inboxes
  has_many :messages, as: :sender, dependent: :nullify
  has_many :copilot_threads, dependent: :destroy_async
  has_many :scenarios, class_name: 'Tekomi::Scenario', dependent: :destroy_async
  has_many :agent_sessions, class_name: 'Tekomi::AgentSession', dependent: :destroy_async
  ...
  validates_with Tekomi::AudienceValidator
  ...
  def responds_to_audience?(contact, conversation)
    ...
    Tekomi::AudienceMatcher.new(config['audience']).matches?(contact, conversation)
  end
  ...
  def auto_resolve_mode
    config.fetch('auto_resolve_mode') { account&.tekomi_auto_resolve_mode || 'evaluated' }
  end
  ...
  def inactivity_threshold_minutes
    return DEFAULT_INACTIVITY_THRESHOLD_MINUTES unless account.feature_enabled?('tekomi_integration_v2')

    (config['auto_resolve_after'] || DEFAULT_INACTIVITY_THRESHOLD_MINUTES).to_i
  end

  def send_inactivity_resolution_message
    return true unless account.feature_enabled?('tekomi_integration_v2')
  ...
  def available_agent_tools
    ...
    custom_tools = account.tekomi_custom_tools.enabled.map(&:to_tool_metadata)
  ...
  def set_default_auto_resolve_mode
    ...
    self.auto_resolve_mode = account&.tekomi_auto_resolve_mode || 'evaluated'
  end
  ...
  def agent_tools
    [
      ...
      *account.tekomi_custom_tools.enabled.map { |custom_tool| custom_tool.tool(self) }
    ]
  end
  ...
  def default_avatar_url
    "#{ENV.fetch('FRONTEND_URL', nil)}/assets/images/dashboard/tekomi/logo.svg"
  end
end
