...
RSpec.describe 'Installation::Onboarding API', type: :request do
  ...
  describe 'GET /installation/onboarding' do
    context 'when TEKOMI_INSTALLATION_ONBOARDING redis key is not set' do
      it 'redirects back' do
        expect(Redis::Alfred.get(Redis::Alfred::TEKOMI_INSTALLATION_ONBOARDING)).to be_nil
        get '/installation/onboarding'
        expect(response).to have_http_status(:redirect)
      end
    end

    context 'when TEKOMI_INSTALLATION_ONBOARDING redis key is set' do
  ...
  describe 'POST /installation/onboarding' do
    ...
    context 'when onboarding is not successfull' do
      it 'does not deletes the redis key' do
        ...
        post '/installation/onboarding', params: { user: {} }
        expect(Redis::Alfred.get(Redis::Alfred::TEKOMI_INSTALLATION_ONBOARDING)).not_to be_nil
      end
    end
  end
end
